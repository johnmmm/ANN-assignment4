...
class BasicLSTMCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or "basic_lstm_cell", reuse=self._reuse):
            ...
            W = tf.get_variable('weight1', [inputs_state.shape[1], 3*self._num_units], tf.float32)
            b = tf.get_variable('bias1', 3*self._num_units, tf.float32)
            new_state = tf.matmul(inputs_state, W)+b

            arrays = tf.split(value=new_state, num_or_size_splits=3, axis=1)    #将那个U合并在一起了
            i = tf.sigmoid(arrays[0])
            o = tf.sigmoid(arrays[1])
            g = self._activation(arrays[2])

            W_f = tf.get_variable('weight_forget', [inputs_state.shape[1], self._num_units], tf.float32)
            b_f = tf.get_variable('bias_forget', self._num_units, tf.float32, initializer = tf.constant_initializer(self._forget_bias))
            forget_gate = tf.matmul(inputs_state, W_f)+b_f
